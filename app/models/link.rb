class Link < ActiveRecord::Base
  ...
  def self.category_links(category, number_links_in_group = nil)
    if number_links_in_group
      Link.where(category_id: category, published: true).limit(number_links_in_group).order('created_at DESC')
    else
      Link.where(category_id: category, published: true).order('created_at DESC')
    end
  end
